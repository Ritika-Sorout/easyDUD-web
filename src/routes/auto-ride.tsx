...
export const Route = createFileRoute("/auto-ride")({
  head: () => ({
    meta: [
      { title: "Book an Auto — easyDUD" },
      { name: "description", content: "Book metered auto-rickshaws across your city — no-haggle fares, verified drivers and live tracking." },
    ],
  }),
  component: AutoPage,
});

/** Vehicle type sent to the backend for this page. */
const VEHICLE_TYPE = "auto";

type AutoId = "shared" | "standard" | "xl" | "electric";
...
const AUTOS: {
  ...
  { id: "shared", name: "Shared Auto", blurb: "Split the fare", seats: 3, eta: "3 min", price: 35, icon: Users },
  { id: "standard", name: "Standard Auto", blurb: "Classic three-wheeler", seats: 3, eta: "2 min", price: 49, icon: Car },
  { id: "xl", name: "Auto XL", blurb: "Extra room & bags", seats: 4, eta: "5 min", price: 79, icon: Truck },
  { id: "electric", name: "E-Auto", blurb: "Zero emissions", seats: 3, eta: "4 min", price: 59, icon: Zap },
];

const FEATURES = [
  { icon: ShieldCheck, title: "Verified Drivers", body: "Background-checked, top-rated auto partners on every trip." },
  { icon: Navigation, title: "Live Tracking", body: "Share your route in real time with friends and family." },
  { icon: Wallet, title: "No-Haggle Fares", body: "Metered, surge-free pricing — pay exactly what's fair." },
  { icon: Headphones, title: "24/7 Support", body: "A human team on standby whenever you need help." },
];

const RECENT = [
  { city: "Koramangala → HSR", fare: 62, when: "Yesterday, 9:14 AM" },
  { city: "Metro Stn → Office", fare: 45, when: "Mon, 6:30 AM" },
  { city: "Market → Home", fare: 38, when: "Sun, 8:42 PM" },
  { city: "Station → College", fare: 54, when: "Fri, 7:10 PM" },
];
...
function AutoPage() {
  ...
  const [ride, setRide] = useState<AutoId>("standard");
  const [when, setWhen] = useState<"now" | "later">("now");

  const selected = AUTOS.find((r) => r.id === ride)!;
  ...
  const book = () => {
    // vehicleType "auto" is what would be sent to the booking API.
    toast.success(`${selected.name} requested`, {
      description: `${pickup} → ${drop} • ${VEHICLE_TYPE} • ETA ${selected.eta}`,
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bg, fontFamily: FONT_SANS, color: COLORS.ink }}>
        ...
        >
          <div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 22 }}>
              <FloatingBadge label="Live • 320 autos nearby" tone="blue" />
              <FloatingBadge icon={Sparkles} label="Metered • no surge" tone="neutral" />
            </div>
            <h1
              style={{
            ...
            >
              Hail metered auto-rickshaws with no-haggle pricing, verified drivers and real-time tracking.
              Quick, nimble city hops — beautifully simple.
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 24, marginTop: 28 }}>
              {[
                { k: "4.88", v: "Driver rating" },
                { k: "800K+", v: "Rides this month" },
                { k: "90s", v: "Avg arrival" },
              ].map((s) => (
                <div key={s.v}>
          ...
          <GlassCard padding={26} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
              <AnimatedInput icon={Tag} label="Promo code" value={promo} onChange={setPromo} placeholder="AUTO10" />
              <div
                style={{
                  display: "flex",
                  alignItems: "center",
            ...
            <div style={{ marginTop: 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              ...
              <GradientButton icon={ArrowRight} onClick={book}>Search autos</GradientButton>
            </div>
          </GlassCard>
        </div>
      </section>
      ...
      <section style={{ padding: "72px 24px 0" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <SectionHeading eyebrow="Choose your auto" title="A ride for every errand" subtitle="From shared hops to roomy XL autos — pick the option that fits the trip." />
          ...
          <GlassCard padding={0} style={{ overflow: "hidden", minHeight: 360 }}>
            <div style={{ position: "relative", height: "100%", minHeight: 360 }}>
              ...
              <div style={{ position: "absolute", top: 18, left: 18, display: "flex", gap: 8, flexWrap: "wrap" }}>
                <FloatingBadge label="Auto 1.1 km away" />
                <FloatingBadge icon={Clock} label="ETA 3 min" tone="neutral" />
              </div>
              <div style={{ position: "absolute", bottom: 18, right: 18 }}>
                <FloatingBadge icon={Sparkles} label="Metered" tone="green" />
              </div>
            </div>
          </GlassCard>
      ...
      <section style={{ padding: "96px 24px 0" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <SectionHeading eyebrow="Why riders choose us" title="Fair where it matters" />
          <div
            style={{
              marginTop: 32,
              display: "grid",
